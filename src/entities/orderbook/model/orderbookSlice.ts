...
const updateOrderbook = (currentBids: [string, string][], currentAsks: [string, string][], updates: { b: [string, string][], a: [string, string][] }) => {
	...
	const newBids = update(currentBids, updates.b).sort((a, b) => parseFloat(b[0]) - parseFloat(a[0]));
	const newAsks = update(currentAsks, updates.a).sort((a, b) => parseFloat(a[0]) - parseFloat(b[0]));
	...
	reducers: {
		...
		handleOrderbookUpdate: (state, action: PayloadAction<OrderbookUpdate>) => {
			...
			if (update.U <= state.lastUpdateId + 1 && update.u >= state.lastUpdateId + 1) {
				const { newBids, newAsks } = updateOrderbook(state.data.bids, state.data.asks, { b: update.b, a: update.a });
...
export const { handleOrderbookUpdate, resetOrderbookState } = orderbookSlice.actions;
