...
import { useCheckApiKeysMutation } from "@/entities/user/model/userApi";
...
export default function Page() {
  ...
  const [checkApiKeys, { isLoading }] = useCheckApiKeysMutation();
  ...
  const {
    ...
    defaultValues: {
      publicKey: process.env.NEXT_PUBLIC_BINANCE_API_KEY,
      secretKey: process.env.NEXT_PUBLIC_BINANCE_SECRET_KEY,
    },
  });

  const onSubmit = async (data: LoginFormValues) => {
    const result = await checkApiKeys(data);

    if ("data" in result && result.data?.success) {
      router.push("/home");
    } else if ("error" in result) {
      const apiError = result.error as { data?: ApiErrorResponse };
      setError("root", {
        message: apiError.data?.message || "Invalid API keys or network error.",
      });
    }
  };
  ...
  return (
    <div className="bg-bg flex h-full items-center justify-center">
      <div className="border-Line min-h-[580px] w-[425px] rounded-[24px] border p-10">
        <div className="mb-8">
          <svg
            height="28"
            width="140"
        ...
        <div className="mb-8 text-[32px] font-bold">Войти</div>
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
          ...
          <div>
            <textarea
              ...
              className="placeholder:text-TertiaryText caret-PrimaryYellow text-PrimaryText border-InputLine selection:bg-PrimaryYellow focus-within:border-PrimaryYellow flex h-[120px] w-full min-w-0 cursor-text resize-none items-center rounded-md border bg-transparent p-2 px-2 leading-normal transition-colors duration-300 outline-none placeholder:text-[13px] placeholder:font-medium md:placeholder:text-[14px]"
            />
            {errors.secretKey && (
              <p className="mt-1 text-xs text-red-500">
