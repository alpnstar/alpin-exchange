...
export const Orderbook = ({className}: {className?: string}) => {
	const {bids, asks, isLoading,isFetching, isError, maxTotalBids, maxTotalAsks} = useOrderbook('BTCUSDT');

	return (
		<div className={`rounded-md bg-bg p-2 lg:row-start-2 lg:row-end-4 ${className}`}>
			{isFetching ? <div>Loading...</div> : isError ? <div>Error loading data</div> : !!bids.length && !!asks.length ? (<div className="h-full flex flex-col">
